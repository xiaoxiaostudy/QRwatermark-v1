#-----------form https://github.com/XuandongZhao/WatermarkAttacker--------
...
#we add it      
class KLVAEWMAttacker(WMAttacker):
    def __init__(self, f=4, device='cpu'):
        if f == 4:
            ddconfig={"double_z": True,"z_channels": 3,"resolution": 256,"in_channels": 3,"out_ch": 3,"ch": 128,"ch_mult":[1,2,4],"num_res_blocks": 2,"attn_resolutions": [],"dropout": 0.0}
            lossconfig={"target": "diffusion.ldm.modules.losses.LPIPSWithDiscriminator","params":{"disc_start": 50001,"kl_weight": 1.0e-06,"disc_weight": 0.5}}
            klvae = AutoencoderKL(ddconfig,lossconfig,embed_dim=3,ckpt_path="ckpt/klf4.ckpt")
        elif f == 8:
            ddconfig={"double_z": True,"z_channels": 4,"resolution": 256,"in_channels": 3,"out_ch": 3,"ch": 128,"ch_mult":[1,2,4,4],"num_res_blocks": 2,"attn_resolutions": [],"dropout": 0.0}
            lossconfig={"target": "diffusion.ldm.modules.losses.LPIPSWithDiscriminator","params":{"disc_start": 50001,"kl_weight": 1.0e-06,"disc_weight": 0.5}}
            klvae = AutoencoderKL(ddconfig,lossconfig,embed_dim=4,ckpt_path="ckpt/klf8.ckpt")
        elif f == 16:
            ddconfig={"double_z": True,"z_channels": 16,"resolution": 256,"in_channels": 3,"out_ch": 3,"ch": 128,"ch_mult":[1,1,2,2,4],"num_res_blocks": 2,"attn_resolutions": [16],"dropout": 0.0}
            lossconfig={"target": "diffusion.ldm.modules.losses.LPIPSWithDiscriminator","params":{"disc_start": 50001,"kl_weight": 1.0e-06,"disc_weight": 0.5}}
            klvae = AutoencoderKL(ddconfig,lossconfig,embed_dim=16,ckpt_path="ckpt/kl16.ckpt")
        elif f == 32:
            ...
            lossconfig={"target": "diffusion.ldm.modules.losses.LPIPSWithDiscriminator","params":{"disc_start": 50001,"kl_weight": 1.0e-06,"disc_weight": 0.5}}
            klvae = AutoencoderKL(ddconfig,lossconfig,embed_dim=64,ckpt_path="ckpt/kl_f32.ckpt")
        
        self.klvae = klvae.to(device).eval()
        self.device = device
    ...
    def attack(self, image_paths, out_paths):
        for img_path, out_path in zip(image_paths, out_paths):
            ...
            img = img.resize((256, 256), Image.Resampling.LANCZOS)  # 调整为 256x256
            img = transforms.ToTensor()(img).unsqueeze(0).to(self.device)
            
            
            with torch.no_grad():
                attack_images, _ = self.klvae(img)
            
            
            attack_images = attack_images.clamp(0, 1)
            attacked_image = transforms.ToPILImage()(attack_images.squeeze().cpu())
            attacked_image = attacked_image.resize(original_size, Image.Resampling.LANCZOS)
            attacked_image.save(out_path)

class VQVAEWMAttacker(WMAttacker):
    def __init__(self, f=4, device='cpu'):
        if f == 4:
            ...
            vqvae_aug = VQModel(ddconfig, n_embed=8192, embed_dim=3, ckpt_path='ckpt/vq-f4.ckpt')
        elif f == 8:
            ...
            vqvae_aug = VQModel(ddconfig, n_embed=16384, embed_dim=4, ckpt_path='vq_f8.ckpt')
        elif f == 16:
            ...
            vqvae_aug = VQModel(ddconfig, n_embed=16384, embed_dim=8, ckpt_path='ckpt/vq_f16.ckpt')
        
        self.vqvae_aug = vqvae_aug.to(device).eval()
        self.device = device
    ...
    def attack(self, image_paths, out_paths):
        for img_path, out_path in zip(image_paths, out_paths):
            ...
            vimage, _, _ = self.vqvae_aug.encode(img)
            attack_images = self.vqvae_aug.decode(vimage)
